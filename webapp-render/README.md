...
> 📖 项目主 README 见仓库根目录 [../README.md](../README.md)。

## 使用方式

1. 推荐使用 macOS 的 [start.command](./start.command)
2. 或使用 Windows 的 [start.bat](./start.bat)
3. 脚本会自动创建项目内虚拟环境，优先使用项目自带离线依赖包安装 `rdkit`，然后打开 `http://127.0.0.1:8765`
4. 也可以直接双击打开 [index.html](./index.html)

## 桌面封装

- macOS 构建脚本：[build_mac_app.sh](./build_mac_app.sh)
- Windows 构建脚本：[build_windows_exe.bat](./build_windows_exe.bat)
- PyInstaller 配置：[lattice_studio.spec](./lattice_studio.spec)
- 桌面启动器入口：[desktop_launcher.py](./desktop_launcher.py)
